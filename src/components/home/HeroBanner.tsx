...
export default function HeroBanner() {
  ...
  useEffect(() => {
    supabase.from('banner').select('*').order('created_at').then(({ data }) => {
      const list = (data ?? []).filter(b => b.image_url)
      setBanners(list)
      setLoading(false)
      list.forEach(b => {
        if (b.game_id) {
          fetch(`/api/football/live?id=${b.game_id}`)
            .then(r => r.json())
            .then(d => {
              if (d.response?.[0]) setGames(prev => ({ ...prev, [b.id]: d.response[0] }))
            })
            .catch(() => {})
        }
      })
  ...
  if (loading) {
    return (
      <section className="w-full rounded-2xl border border-[#2A2A3A] bg-[#12121A] animate-pulse h-48 sm:h-72 md:h-96 lg:h-[480px]" />
    )
  }
  ...
  const game = games[banner?.id ?? ''] ?? null
  ...
  return (
    <section className="rounded-2xl border border-[#2A2A3A] overflow-hidden">

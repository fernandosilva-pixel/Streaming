...
async function ping(url: string, timeoutMs = 5000): Promise<{ ok: boolean; latencyMs: number; status?: number }> {
  ...
  try {
    ...
    clearTimeout(timer)
    return { ok: res.ok || res.status < 500, latencyMs: Date.now() - start, status: res.status }
  } catch {

                ...
                <div className="space-y-6">
                  ...
                  {(() => {
                    ...
                    for (const p of dashPayments.filter(p => p.status === 'PAID')) {
                      ...
                      byUser[key].push({
                        amount: p.amount ?? 0,
                        label: p.stream_title || streams.find(s => s.id === p.stream_id)?.title || 'Jogo avulso',
                        date: p.created_at ?? '',
                      })
                    }
        ...
        <div className="space-y-4">
          ...
          {(() => {
            ...
            for (const p of dashPayments.filter(p => p.status === 'PAID')) {
              const key = normalizeId(p.user_phone)
              if (!byUser[key]) byUser[key] = []
              byUser[key].push({ amount: p.amount ?? 0, label: p.stream_title || streams.find(s => s.id === p.stream_id)?.title || 'Jogo avulso', date: p.created_at ?? '' })
            }
            for (const p of dashPlanPayments.filter(p => p.status === 'PAID')) {
              const key = normalizeId(p.user_email)
              if (!byUser[key]) byUser[key] = []
